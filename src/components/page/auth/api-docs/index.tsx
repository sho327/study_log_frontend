      ...
      <Stack gap="lg">
        <Box>
          ...
          <Text c="dimmed" size="sm">
            Use the StudyLog API to post logs from CLI, GitHub Actions, or other tools.
          </Text>
        </Box>
...
jobs:
  log:
    ...
    steps:
      - name: Post study log
        env:
          STUDYLOG_TOKEN: \$\{\{ secrets.STUDYLOG_TOKEN \}\}
          REPO_NAME: \$\{\{ github.repository \}\}
          REPO_URL: \$\{\{ github.event.repository.html_url \}\}
        run: |
          curl -X POST https://your-domain.com/api/logs/create \\
            -H "Content-Type: application/json" \\
            -H "Authorization: Bearer $STUDYLOG_TOKEN" \\
            -d '{
              "userId": "your-user-id",
              "date": "'$(date +%Y-%m-%d)'",
              "duration": 30,
...
export function AuthApiDocsContent() {
