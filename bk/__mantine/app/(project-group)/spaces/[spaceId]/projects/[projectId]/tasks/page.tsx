...
export default function TaskListPage() {
  // 1. 各行（Row）の生成ロジック：空白ノードエラーを避けるため構造を厳密に管理
      ...
      <Table.Tr key={task.id}>
        ...
        <Table.Td style={{ width: rem(130) }}>
          <Group gap={6} wrap="nowrap">
            <IconCalendarMonth style={{ width: 16, height: 16 }} color={isOverdue ? 'var(--mantine-color-red-6)' : 'gray'} />
        ...
        <Table.Td style={{ width: rem(40) }}>
          <Menu id={`task-menu-${task.id}`} shadow="md" position="bottom-end" withinPortal>
            <Menu.Target>
              <ActionIcon variant="subtle" color="gray" size="lg"><IconDotsVertical style={{ width: 18, height: 18 }} /></ActionIcon>
            </Menu.Target>
            <Menu.Dropdown>
              <Menu.Label>タスク操作</Menu.Label>
              <Menu.Item leftSection={<IconEdit style={{ width: 14, height: 14 }} />}>編集</Menu.Item>
              <Divider />
              <Menu.Item color="red" leftSection={<IconTrash style={{ width: 14, height: 14 }} />}>削除</Menu.Item>
            </Menu.Dropdown>
          </Menu>
        </Table.Td>
      </Table.Tr>
    ...
    <Box style={{ width: '100%' }}>
      <Stack gap="xl">
        ...
        <Group justify="space-between" align="flex-end">
          <Stack gap={4}>
            <Group gap="xs">
              <IconChecklist style={{ width: 28, height: 28 }} color="var(--mantine-color-brand-6)" />
          ...
          <Button 
            ...
            leftSection={<IconPlus style={{ width: 18, height: 18 }} />} 
          >
            新規タスク作成
          </Button>
        </Group>
        ...
        >
          ...
          <Box p="md" style={{ backgroundColor: 'oklch(0.98 0.001 264)' }}>
            <Group justify="space-between">
              ...
              />
              <Group gap="xs">
                <Button variant="light" color="gray" size="sm" leftSection={<IconFilter style={{ width: 14, height: 14 }} />} radius="md">フィルタ</Button>
